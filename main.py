...
try:
    ...
    print(e)
    print(Fore.RED + f'[FEHLER] ' + Fore.RESET +
          'Fehler Beim Starten des Bots Bitte Starte "INSTALL PACKAGES.bat')
    exit()
...
intro = Fore.LIGHTGREEN_EX + """
  _               _             ____        ____                              
 | |    ___  __ _| | _____ _ __| __ ) _   _|  _ \ _ __ __ _  __ _  ___  _ __  
 | |   / _ \/ _` | |/ / _ \ '__|  _ \| | | | | | | '__/ _` |/ _` |/ _ \| '_ \ 
 | |__|  __/ (_| |   <  __/ |  | |_) | |_| | |_| | | | (_| | (_| | (_) | | | |
 |_____\___|\__,_|_|\_\___|_|  |____/ \__, |____/|_|  \__,_|\__, |\___/|_| |_|
                                      |___/                 |___/             
"""
...
print(Fore.BLUE + f'[Credits] ' + Fore.RESET +
      'Credits Xenis Discord: https://discord.gg/KCwScTMpjH')
...
def getNewSkins():
    ...
    for cid in [
            ...
            if item.split('/')[-1].upper().startswith('CID_')
    ]:
...
def getNewEmotes():
    ...
    for cid in [
            item for item in response
            if item.split('/')[-1].upper().startswith('EID_')
    ]:
...
with open('config.json') as f:
    try:
        ...
        print(
            Fore.RED + ' [FEHLER] ' + Fore.RESET +
            "In einer der Bot-Dateien ist ein Fehler aufgetreten! (config.json). Wenn Sie Probleme haben, das Problem zu beheben, treten Sie dem Discord-Supportserver bei, um Hilfe zu erhalten - https://discord.gg/88r2ShB"
        )
...
with open('info.json') as f:
    try:
        ...
        print(
            Fore.RED + ' [FEHLER] ' + Fore.RESET +
            "In einer der Bot-Dateien ist ein Fehler aufgetreten! (info.json) Wenn Sie Probleme haben, das Problem zu beheben, treten Sie dem Discord-Supportserver bei, um Hilfe zu erhalten - https://discord.gg/88r2ShB"
        )
        print(Fore.LIGHTRED_EX + f'\n {e}')
        exit(1)
...
client = commands.Bot(
    ...
    auth=fortnitepy.AdvancedAuth(email=data['email'],
...
async def event_ready():
    ...
    print(Fore.GREEN + ' [Success] ' + Fore.RESET +
          'Ajax Bot is Online! ' + Fore.LIGHTBLUE_EX +
    ...
    client.set_avatar(
        fortnitepy.Avatar(asset=data['cid'],
                          background_colors=['#ffffff', '#ee1064', '#ff0000']))
...
async def event_party_invite(invite):
    if data['joinoninvite'].lower() == 'true':
        try:
            await invite.accept()
            print(
                Fore.BLUE + ' [Console] ' + Fore.RESET +
                f'Partyeinladung akzeptiert von {invite.sender.display_name}')
        except Exception:
            pass
    elif data['joinoninvite'].lower() == 'false':
        if invite.sender.id in info['FullAccess']:
            await invite.accept()
            print(Fore.BLUE + ' [Console] ' + Fore.RESET +
                  'Partyeinladung akzeptiert von ' + Fore.LIGHTGREEN_EX +
                  f'{invite.sender.display_name}')
        else:
            print(
                Fore.RED + ' [Console] ' + Fore.RESET +
                f'Partyeinladung von nie angenommen von {invite.sender.display_name}'
            )


@client.command()
async def pinkghoul(ctx):
    skin_variants = client.party.me.create_variants(material=3)

    await client.party.me.set_outfit(
        asset='CID_029_Athena_Commando_F_Halloween', variants=skin_variants)
...
async def event_friend_request(request):
    if data['friendaccept'].lower() == 'true':
        try:
            ...
            print(
                f' [Console] Akzeptierte Freundschaftsanfrage von {request.display_name}'
                + Fore.LIGHTGREEN_EX + f' ({lenFriends()})')
        except Exception:
            pass
        ...
        if request.id in info['FullAccess']:
            try:
                await request.accept()
                print(Fore.BLUE + ' [Console] ' + Fore.RESET +
                      'Akzeptierte Freundschaftsanfrage von ' +
                      Fore.LIGHTGREEN_EX + f'{request.display_name}' +
                      Fore.LIGHTBLUE_EX + f' ({lenFriends()})')
            except Exception:
                pass
        else:
            print(
                f' [Console] Nie angenommene Freundschaftsanfrage von {request.display_name}'
            )
    ...
    if client.user.display_name != member.display_name:
        try:
            if client.user.id in info['FullAccess']:
                print(Fore.LIGHTBLUE_EX + f' [Lobby] {member.display_name}' +
                      Fore.RESET + 'ist der Lobby beigetreten.')
            else:
                print(
                    f' [Lobby] {member.display_name} ist der Lobby beigetreten.'
                    + Fore.LIGHTGREEN_EX + f' ({lenPartyMembers()})')
        except fortnitepy.HTTPException:
            pass
        await client.party.send(
            f' Welcome {member.display_name}! Bot Made By AjaxFN And Subcribe to AjaxFortn1te on yt also the version is 1.4 new 24/7 bot up time soon'
        )
...
async def event_party_member_leave(member):
    if client.user.display_name != member.display_name:
        try:
            if client.user.id in info['FullAccess']:
                print(Fore.LIGHTBLUE_EX + f' [Lobby] {member.display_name}' +
                      Fore.RESET + 'hat die Lobby verlassen.')
            else:
                print(
                    f' [Lobby] {member.display_name} hat die Lobby verlassen.'
                    + Fore.LIGHTGREEN_EX + f' ({lenPartyMembers()})')
        except fortnitepy.HTTPException:
            pass
...
async def event_party_message(message):
    ...
    print(Fore.LIGHTGREEN_EX + ' [Chat] ' + f'{name}' + Fore.RESET +
          f': {message.content}')
...
async def event_friend_message(message):
    if message.author.id in info['FullAccess']:
        ...
        name = Fore.RESET + f'{message.author.display_name}'
    print(Fore.LIGHTMAGENTA_EX + ' [Flüstern] ' + f'{name}' + Fore.RESET +
          f': {message.content}')
...
async def backpack(ctx, *, content=None):
    if content is None:
        await ctx.send(
            f'No backpack was given, try: {prefix}backpack (backpack name)')
...
async def pickaxe(ctx, *, content=None):
    if content is None:
        await ctx.send(
            f'No pickaxe was given, try: {prefix}pickaxe (pickaxe name)')
...
async def random(ctx, content=None):
    ...
    skins = await BenBotAsync.get_cosmetics(lang="en",
                                            backendType="AthenaCharacter")

    skin = rand.choice(skins)

    backpacks = await BenBotAsync.get_cosmetics(lang="en",
                                                backendType="AthenaBackpack")

    backpack = rand.choice(backpacks)

    emotes = await BenBotAsync.get_cosmetics(lang="en",
                                             backendType="AthenaDance")

    emote = rand.choice(emotes)

    pickaxes = await BenBotAsync.get_cosmetics(lang="en",
                                               backendType="AthenaPickaxe")
...
async def rr(ctx):
    ...
    await ctx.send('Skin set to: Renegade raider')


@client.command()
async def purpleportal(ctx):
    variants = client.party.me.create_variants(item='AthenaBackpack',
                                               particle_config='Particle',
                                               particle=1)

    await client.party.me.set_backpack(asset='BID_105_GhostPortal',
                                       variants=variants)
...
async def new(ctx, content=None):
    ...
    if content is None:
        await ctx.send(
            f'There are {len(newSkins) + len(newEmotes)} new skins + emotes')
...
async def copy(ctx, *, username=None):
    ...
    await client.party.me.edit_and_keep(
        ...
        partial(fortnitepy.ClientPartyMember.set_emote, asset=member.emote))
...
async def avatar(ctx, *, skin=None):
    if skin is None:
        await ctx.send(
            f'No skin was given. Try: {prefix}avatar (skin name, cid)')
...
@client.command()
@is_admin()
async def tell(ctx, member=None, *, message=None):
    ...
    if players is not None:
        if 'auto' in players.lower():
            if client.party.me.in_match():
                ...
                left = 100
            await client.party.me.set_in_match(players_left=left,
                                               started_at=time)
            ...
            while (client.party.me.match_players_left <=
                   5) and (client.party.me.match_players_left > 3):
                await client.party.me.set_in_match(
                    players_left=client.party.me.match_players_left -
                    rand.randint(1, 2),
                    started_at=time)
            ...
            while (client.party.me.match_players_left <=
                   3) and (client.party.me.match_players_left > 1):
                await client.party.me.set_in_match(
                    players_left=client.party.me.match_players_left - 1,
                    started_at=time)
            ...
            try:
                await client.party.me.set_in_match(players_left=int(players),
                                                   started_at=time)
            except ValueError:
...
async def kick(ctx: fortnitepy.ext.commands.Context, *, member=None):
    try:
        ...
        await ctx.send("I can't kick that user because I am not party leader lol")
    except AttributeError:
...
async def join(ctx, *, member=None):
    try:
        ...
        await ctx.send(
            "I can not join that party. Are you sure I have them friended?")
        ...
        try:
            ...
            if user.id in friends:
                ...
                print(Fore.GREEN + ' [+] ' + Fore.RESET +
                      'Sent a friend request to: ' + Fore.LIGHTBLACK_EX +
                      f'{user.display_name}')
        ...
        try:
            ...
            if user.id in friends:
                try:
                    ...
                    await ctx.send(
                        "Something went wrong trying to block that user.")
...
async def unblock(ctx, *, user=None):
    if user is not None:
        try:
            ...
            if member.id in blocked:
                try:
                    ...
                    await ctx.send(
                        f'Successfully unblocked {member.display_name}')
                except fortnitepy.HTTPException:
                    await ctx.send(
                        'Something went wrong trying to unblock that user.')
            else:
        ...
        await ctx.send(
            f'No user was given. Try: {prefix}unblock (blocked user)')
...
async def friends(ctx):
    ...
    try:
        ...
        await ctx.send(
            f"Client has: {len(onlineFriends)} friends online and {len(offlineFriends)} friends offline"
        )
        await ctx.send("(Check cmd for full list of friends)")

        print(" [+] Friends List: " + Fore.GREEN +
              f'{len(onlineFriends)} Online ' + Fore.RESET + "/" +
...
@client.command()
async def invis(ctx: fortnitepy.ext.commands.Context):
...
async def id(ctx, *, user=None):
    ...
    try:
        ...
        print(Fore.GREEN + ' [+] ' + Fore.RESET + f"{user}'s Epic ID is: " +
              Fore.LIGHTBLACK_EX + f'{user.id}')
    except AttributeError:
        await ctx.send("I couldn't find an Epic account with that name.")


#More Commands By AjaxFN ;)
...
async def user(ctx, *, user=None):
    if user is not None:
        ...
        try:
            ...
            print(Fore.GREEN + ' [+] ' + Fore.RESET +
                  f'The ID: {user.id} belongs to: ' + Fore.LIGHTBLACK_EX +
                  f'{user.display_name}')
        except AttributeError:
...
async def Test(ctx, *, content=None):
    await client.party.me.set_outfit(
        asset='CID_636_Athena_Commando_M_GalileoGondola_78MFZ')
...
async def Tbd(ctx, *, content=None):
    await client.party.me.set_outfit(
        asset='CID_568_Athena_Commando_M_RebirthSoldier')
    ...
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(
        asset='CID_NPC_Athena_Commando_F_CloakedAssassin')
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(
    ...
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(
        asset='CID_NPC_Athena_Commando_F_TowerSentinel')
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(
        asset='CID_NPC_Athena_Commando_M_AlienRobot')
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(asset='CID_NPC_Athena_Commando_M_Broccoli'
                                     )
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(
        asset='CID_NPC_Athena_Commando_M_CavernArmored')
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(
        asset='CID_NPC_Athena_Commando_M_EmperorSuit')
    await asyncio.sleep(4.0)
    ...
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(
        asset='CID_TBD_Athena_Commando_F_BuffetCine')
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(
    ...
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(
        asset='CID_TBD_Athena_Commando_M_Nutcracker_CINE')
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(
        asset='CID_TBD_Athena_Commando_M_Turtleneck_EVENT_NOTFORSTORE')
...
async def Og(ctx, *, content=None):
    ...
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(
        asset='CID_082_Athena_Commando_M_Scavenger')
    await asyncio.sleep(4.0)
...
@client.command()
async def exlu(ctx, *, content=None):
    await client.party.me.set_outfit(
    ...
    await asyncio.sleep(4.0)
    await client.party.me.set_outfit(
        asset='CID_783_Athena_Commando_M_AquaJacket')
    await ctx.send(f'those were all exclusive skins')


@client.command()
async def ajax(ctx, *, content=None):
    if content is None:
        await ctx.send(
            f'Info: Bot Made By AJAXFN, subcribe to my youtube @ajaxfortn1te!'
        )
        await client.party.me.set_outfit(
            asset='CID_A_063_Athena_Commando_F_CottonCandy')
...
async def Randomize(ctx, *, content=None):
    if content is None:
        ...
        await client.party.me.set_outfit(
            asset='CID_042_Athena_Commando_M_Cyberpunk')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_043_Athena_Commando_F_Stealth')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_044_Athena_Commando_F_SciPop')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_045_Athena_Commando_M_HolidaySweater')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_046_Athena_Commando_F_HolidaySweater')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_047_Athena_Commando_F_HolidayReindeer')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_048_Athena_Commando_F_HolidayGingerbread')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_049_Athena_Commando_M_HolidayGingerbread')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_050_Athena_Commando_M_HolidayNutcracker')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_051_Athena_Commando_M_HolidayElf')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_052_Athena_Commando_F_PSBlue')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_053_Athena_Commando_M_SkiDude')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_054_Athena_Commando_M_SkiDude_USA')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_070_Athena_Commando_M_Cupid')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_071_Athena_Commando_M_Wukong')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_072_Athena_Commando_M_Scout')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_073_Athena_Commando_F_Scuba')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_074_Athena_Commando_F_Stripe')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_075_Athena_Commando_F_Stripe')
        await asyncio.sleep(0.001)
        ...
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_080_Athena_Commando_M_Space')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_081_Athena_Commando_F_Space')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_082_Athena_Commando_M_Scavenger')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_083_Athena_Commando_F_Tactical')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_084_Athena_Commando_M_Assassin')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_085_Athena_Commando_M_Twitch')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_086_Athena_Commando_M_RedSilk')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_089_Athena_Commando_M_RetroGrey')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_096_Athena_Commando_F_Founder')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_097_Athena_Commando_F_RockerPunk')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_098_Athena_Commando_F_StPatty')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_099_Athena_Commando_F_Scathach')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_100_Athena_Commando_M_CuChulainn')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_101_Athena_Commando_M_Stealth')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_102_Athena_Commando_M_Raven')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_103_Athena_Commando_M_Bunny')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_104_Athena_Commando_F_Bunny')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_105_Athena_Commando_F_SpaceBlack')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(asset='CID_106_Athena_Commando_F_Taxi'
                                         )
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_107_Athena_Commando_F_PajamaParty')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_108_Athena_Commando_M_Fishhead')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_109_Athena_Commando_M_Pizza')
        await asyncio.sleep(0.001)
        await client.party.me.set_outfit(
            asset='CID_043_Athena_Commando_F_Stealth')
        await asyncio.sleep(0.001)
...
async def Hack(ctx, *, member=None):
    if member is None:
        member = [m for m in client.party.members if m.id == ctx.author.id][0]
        await ctx.send(
            f'Hacking Hacking Fortnite User {member.display_name}...')
        await asyncio.sleep(1.0)
...
async def SpecialCommand(ctx, *, content=None):
    if content is None:
        await ctx.send(
            f'{prefix}ajax , {prefix}Randomize, {prefix}Spam, {prefix}Hack, {prefix}og, {prefix}Exlu, {prefix}RE, {prefix}Toxic, {prefix}Rdw, {prefix}Johnnysins, {prefix}Info, {prefix}battlepassemotes '
        )
...
async def JohnnySins(ctx, *, content=None):
    if content is None:
        await client.party.me.set_outfit(
            asset='CID_989_Athena_Commando_M_ProgressiveJonesy_Event')
...
async def Info(ctx, *, content=None):
    if content is None:
        ...
        await ctx.send(f'Bot Made By AjaxFN')
        await ctx.send(f'Bot File AJAXBOT')


@client.command()
async def Battlepassemotes(ctx, *, content=None):
    if content is None:
...
async def Toxic(ctx, *, content=None):
    if content is None:
        ...
        await asyncio.sleep(2.0)
        await ctx.send(f'Come 1 against 1 you noob I beat you easy hahahahah')
    ...
    if (setting is None) and (user is None):
            ...
            if user.id not in info['FullAccess']:
                ...
                if (content.lower() == 'yes') or (content.lower() == 'y'):
                    ...
                    with open('info.json', 'w') as f:
                        ...
                        print(Fore.BLUE + " [+] " + Fore.LIGHTBLUE_EX +
                              user.display_name + Fore.RESET +
        ...
        if setting.lower() == 'add':
            if ctx.message.author.id in info['FullAccess']:
                ...
                await ctx.send(
                    "You don't have access to add other people as admins. Try just: !admin add"
                )
        elif setting.lower() == 'remove':
            if ctx.message.author.id in info['FullAccess']:
                if user.id in info['FullAccess']:
                    await ctx.send("Password?")
                    response = await client.wait_for('friend_message',
                                                     timeout=20)
                    content = response.content.lower()
                    if content == data['AdminPassword']:
                        info['FullAccess'].remove(user.id)
...
if (data['email'] and data['password']) and (data['email'] != ""
                                             and data['password'] != ""):
    try:
        client.run()
        ...
        print(e)
        print(Fore.RED + f'[FEHLER] ' + Fore.RESET +
              'Bitte Starte "INSTALL PACKAGES.bat')
        exit()
else:
    print(
        Fore.RED + ' [FEHLER] ' + Fore.RESET +
        'Du hast bei config.jason nicht deine fortnite acc daten eingegeben.')
